...
class Net(nn.Module):
    def __init__(self, n_status, n_action):
        ...
        self.status = nn.Sequential(
            nn.Linear(n_status, hidden_dim*2),
            nn.ReLU(),
...
def load_model(n_status, n_action, pth):
    net = Net(n_status=n_status, n_action=n_action)
...
def train_model(net, time_steps, X_train, y_train, criterion, optimizer):
    # Train the model
...
def test_model(net, XX_test, yy_test, threshold=0.5):
    with torch.no_grad():
        outputs = net(XX_test)
        predicted = torch.argmax(outputs.data, dim=1)
        y_labels = torch.argmax(yy_test, dim=1)
        # Calculate accuracy
...
# Train the model
train_model(net, time_steps=200, X_train=X_train, y_train=y_train, criterion=criterion, optimizer=optimizer)

# Test the model
test_model(net, X_train, y_train, threshold=0.8)
test_model(net, X_test, y_test, threshold=0.1)
